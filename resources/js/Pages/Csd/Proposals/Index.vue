...
function filterByStatus(status) {
    router.get(route('csd.proposals.index'), { status }, { preserveState: true });
}
</script>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <Link :href="route('csd.proposals.create')"
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="proposal in proposals.data" :key="proposal.id">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <Link :href="route('csd.proposals.show', proposal.id)" class="text-sm font-medium text-gray-900 hover:text-blue-600">
                                                {{ proposal.title }}
                                            </Link>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <Link :href="route('csd.proposals.show', proposal.id)" class="text-indigo-600 hover:text-indigo-900 mr-4">
                                                View
                                            </Link>
                                            <Link v-if="proposal.status === 'draft'" :href="route('csd.proposals.submit', proposal.id)"
                                    ...
                                    <tr v-if="proposals.data.length === 0">
                                        <td colspan="6" class="px-6 py-12 text-center text-gray-500">
                                            No proposals found. <Link :href="route('csd.proposals.create')" class="text-blue-600 hover:text-blue-900">Create one?</Link>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
